...
static void ip_dump(const uint8_t *data, size_t len){
    ...
    flockfile(stderr); //dumpの表示中にほかの表示が割り込まないようロック
...
struct ip_iface *ip_iface_alloc(const char *unicast, const char *netmask){
    ...
    if(ip_addr_pton(unicast, &iface->unicast) == -1){
        errorf("Failed to set unicast address");
        memory_free(iface);
        return NULL;
    }
    if(ip_addr_pton(netmask, &iface->netmask) == -1){
        errorf("Failed to set netmask");
        memory_free(iface);
        return NULL;
    }
    uint32_t broadcast = iface->unicast & iface->netmask;
    broadcast = broadcast | ~iface->netmask;
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev){
    ...
    if(v != IP_VERSION_IPV4){
        ...
        errorf("inputed len is shorter than header len");
        return;
    }
    else if(len < total){
        errorf("inputed len is shorter than total len");
        return;
    }
    else if(cksum16((uint16_t *)hdr, hlen, 0) != 0){
    ...
    uint32_t broadcast;
    ip_addr_pton("255.255.255.255", &broadcast);
    if( iface->unicast != hdr->dst &&
    hdr->dst != broadcast &&
    ...
    for(entry = protocols; entry; entry = entry->next){
        if(hdr->protocol == entry->type){
            entry->handler(data + IP_HDR_SIZE_MIN, len - IP_HDR_SIZE_MIN, hdr->src, hdr->dst, iface);
            return;
        }
    }
